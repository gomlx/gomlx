...
// Const creates constant nodes in the Graph. It can take Local tensors as well as
// multidimensional slices (or scalars). It uses introspection (reflect package) to figure out
// the shape given a Go scalar/slice/array. If the value is unsupported, it sets the error in
// the Graph.
//
// A tensor.Device (e.g., generated by another computation) will be converted to local first.
func Const(g *Graph, x interface{}) *Node {
	localT, ok := x.(*tensor.Local)
	if ok {
		return ConstLocal(g, localT)
	}
	if t, ok := x.(tensor.Tensor); ok {
		return ConstLocal(g, t.Local())
	}

	// Convert Go type to tensor.Local:
	localT = tensor.FromAnyValue(x)
	if localT.Error() != nil {
		g.SetErrorf("failed creating constant node: %w", localT.Error())
