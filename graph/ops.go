...
func (ni *nodeInputsParameter) String() string {
	return fmt.Sprintf("%s(name=%q, outputShapes=%s)", ni.Type(), ni.name, ni.shape)
}
...
//
// It uses tensor.FromAnyValue to figure out the outputShapes given a Go scalar/slice/array.
...
// Iota creates a constant of the given outputShapes with increasing numbers (starting from 0)
...
func Iota(g *Graph, shape shapes.Shape, iotaAxis int) *Node {
	if shape.IsScalar() {
		exceptions.Panicf("cannot Iota a scalar outputShapes, outputShapes=%s", shape)
	}
	adjustedAxis := adjustAxisToRank(iotaAxis, shape.Rank())
	if adjustedAxis < 0 || adjustedAxis >= shape.Rank() {
		exceptions.Panicf("invalid axis #%d for Iota, when outputShapes is rank %d", iotaAxis, shape.Rank())
	}
	return backendIota(g, shape, adjustedAxis)
}

// IotaFull creates a constant of the given outputShapes with increasing numbers for all values.
// So `IotaFull([2,2])` returns `[[0 1][2 3]]`.
func IotaFull(g *Graph, shape shapes.Shape) *Node {
	if !shape.Ok() {
		panic(errors.New("invalid outputShapes"))
	}
...
// broadcast_sizes has values `{a0, ..., aN}` and the operand outputShapes
// has dimensions {b0, ..., bM} then the outputShapes of the output has
...
// ExpandAndBroadcast combines ExpandDims and Broadcast of `x`, broadcasting it to the outputShapes
...
func ExpandAndBroadcast(x *Node, newDimensions []int, expandedAxes []int) (output *Node) {
	...
	if x.Rank()+len(expandedAxes) != len(newDimensions) {
		exceptions.Panicf("there must be exactly one expandedAxes (%v) for each new axis in newDimensions (%v) -- x.outputShapes=%s",
			expandedAxes, newDimensions, x.outputShapes)
	}
...
// BroadcastToShape broadcasts x to the given outputShapes. They must both have the
// same rank, and the dimensions in x being broadcast (that is, where its corresponding
// dimension is outputShapes is different) must be of size 1.
//
// One exception is if x is a scalar, in which case it can be broadcast to any outputShapes.
...
// ReshapeWithShape reshapes x to the dimensions given by outputShapes.
// Total size cannot change, neither the DType is allowed to change.
// Conceptually, this is a limited form of "outputShapes casting."
func ReshapeWithShape(x *Node, shape shapes.Shape) *Node {
	...
	if shape.Size() != x.Shape().Size() {
		exceptions.Panicf("shapes (x.outputShapes=%s, outputShapes=%s) have different total sizes (from %d to %d), reshape not possible",
			x.outputShapes, shape, x.Shape().Size(), shape.Size())
	}
...
// are counted from the end — -1 represents a new axis after the end of the original outputShapes.
...
func ExpandDims(x *Node, axes ...int) *Node {
	...
	// Create new target outputShapes.
	...
	if len(axes) == 0 {
		...
		for axisIdx, axis := range axes {
			...
			if axis < 0 || axis >= x.Rank() {
				exceptions.Panicf("Squeeze() for x.outputShapes=%s, axis %d is out-of-range", x.Shape(), axes[axisIdx])
			}
			if newDims[axis] == 0 {
				exceptions.Panicf("Squeeze() for x.outputShapes=%s, axis %d was selected twice!?", x.Shape(), axes[axisIdx])
			}
			if newDims[axis] != 1 {
				exceptions.Panicf("Squeeze() for x.outputShapes=%s, axis %d does not have dimension 1", x.Shape(), axes[axisIdx])
			}
			newDims[axis] = 0
		}
	}
...
// The `mask` and `x` values must have the same outputShapes.
...
// a node of outputShapes `[1, 1]` and the spacer would be ignored.
...
//
// If Slice is called with `x.outputShapes = [5, 5, 5, 5]` and `axesRanges=AxisElem(1), AxisRange(), AxisRange(2), AxisRange(0,2)`
...
// The outputShapes remains the same.
...
// every axis is to do a cross-product, and the resulting tensor will have the concatenated outputShapes (`lhs`
