...
// ShapeFromXlaShape allocates and sets a new Shape struct set with the same shape defined by xla::Shape. C++ only.
...
xla::StatusOr<xla::XlaComputation> xlaCompForReduction(xla::XlaBuilder *builder, xla::XlaOp &init_op, int32_t node_type) {
    auto shape_or = builder->GetShape(init_op);
    if (!shape_or.ok()) {
        return shape_or.status();
    }
    xla::PrimitiveType primitive_type = shape_or.value().element_type();  // They are both ints.
    ...
    return xla::Status(absl::StatusCode::kInvalidArgument,
       absl::StrFormat("invalid node_type=%d for xlaCompForReduction", node_type));
}
...
XlaStatus* ComputationAddOp(Computation *comp, SerializedNode *node) {
    ...
    // Create new XlaOp.
    // TODO: A registration mechanism, where one can implement different node_types in different files or libraries even.
    xla::XlaOp op;

    // Extract optional parameters.
    xla::XlaOp **inputs = node->inputs;
    absl::Span<const int64_t> list_of_ints(node->integer_array, node->integer_array_size);
    absl::Span<const int64_t> shape_dimensions;
    xla::Shape shape;
    if (node->shape != nullptr) {
        shape_dimensions = absl::Span<const int64_t>(node->shape->dimensions, node->shape->rank);
        shape = MakeXlaShape(node->shape);
    }

    // Tool to decode contents encoded in the integer array.
    int integer_array_pos = 0;
    auto decode = [&integer_array_pos, node]() { return node->integer_array[integer_array_pos++]; };
    auto decodeSpan = [&integer_array_pos, node](int len) {
        const absl::Span<const int64_t> result(node->integer_array+integer_array_pos, len);
    ...
    case ConvertTypeNode:
        op = xla::ConvertElementType(*inputs[0], static_cast<xla::PrimitiveType>(node->integer));
        break;
    case WhereNode:
    ...
    case ReduceMultiplyNode:
    case ReduceMaxNode: {
        auto reduce_comp_or = xlaCompForReduction(builder, *inputs[1], node->node_type);
    ...
    case GatherNode: {
        ...
        for (int ii = 0; ii < len_collapsed_slice_dims; ii++) {
            gather_dims.mutable_collapsed_slice_dims()->Add(node->integer_array[pos++]);
        }
        ...
        absl::Span<const int64_t> slice_sizes(node->integer_array+pos, len_slice_sizes);
        op = xla::Gather(*inputs[0], *inputs[1], gather_dims, slice_sizes, indices_are_sorted);
        break;
    }
    case ScatterNode: {
        int pos = 0;
        ...
        for (int ii = 0; ii < len_update_window_dims; ii++) {
            scatter_dims.mutable_update_window_dims()->Add(node->integer_array[pos++]);
        }
        for (int ii = 0; ii < len_inserted_window_dims; ii++) {
            scatter_dims.mutable_inserted_window_dims()->Add(node->integer_array[pos++]);
        }
        for (int ii = 0; ii < len_scatter_dims_to_operand_dims; ii++) {
            scatter_dims.mutable_scatter_dims_to_operand_dims()->Add(node->integer_array[pos++]);
        }
        ...
        auto update_computation = CreateScalarAddComputation(primitive_type, builder);
        op = Scatter(*inputs[0], *inputs[1], *inputs[2],
                update_computation,
                scatter_dims,
    ...
    case ConvGeneralDilatedNode: {
        ...
        op = ConvGeneralDilated(*inputs[0], *inputs[1],
           window_strides,
           /* absl::Span<const std::pair<int64_t, int64_t>> */ padding,
           input_dilation, filter_dilation,
           conv_dims, filter_group_count, batch_group_count,
           precision_config,
           preferred_element_type, window_reversal);
        break;
    }
    case ReverseNode: {
        op = Rev(*inputs[0], absl::Span<const int64_t>(node->integer_array, node->integer_array_size));
        break;
    }
    case TransposeNode: {
        op = Transpose(*inputs[0], absl::Span<const int64_t>(node->integer_array, node->integer_array_size));
        break;
    }
    case ReduceWindowNode: {
        // Create reduction comp.
        int64_t reduction_type = node->integer;
        auto reduce_comp_or = xlaCompForReduction(builder, *inputs[1], reduction_type);
        if (!reduce_comp_or.ok()) {
            return new xla::Status(reduce_comp_or.status());
        }
        auto reduce_comp = std::move(reduce_comp_or.value());
        ...
        absl::Span<const int64_t> window_dilations = decodeSpan(len_window_dilations);
        ...
        op = xla::ReduceWindowWithGeneralPadding(*inputs[0], *inputs[1], reduce_comp,
            window_dimensions, window_strides, base_dilations, window_dilations, paddings);
        break;
    }
    case SelectAndScatterNode: {
        ...
        xla::PrimitiveType primitive_type = shape_or.value().element_type();  // They are both ints.
        xla::XlaComputation scatter_comp(CreateScalarAddComputation(primitive_type, builder));
        xla::XlaComputation select_comp(CreateScalarGeComputation(primitive_type, builder));
        ...
        op = SelectAndScatterWithGeneralPadding(
            operand, select_comp,
            window_dimensions, window_strides, paddings, source,
            init_value, scatter_comp);
        break;
    }
    case BatchNormInferenceNode: {
        ...
        op = xla::BatchNormInference(operand, scale, offset, mean, variance, epsilon, feature_index);
        break;
    }
    ...
    case BatchNormGradNode: {
        ...
        float epsilon = node->float_v;
        int64_t feature_index = node->integer;
        op = xla::BatchNormGrad(operand, scale, batch_mean, batch_var, grad_output, epsilon, feature_index);
        break;
    }
    case DotGeneralNode: {
        ...
        std::vector<google::protobuf::RepeatedField<google::protobuf::int64>*> lists = {
        ...
        const xla::PrecisionConfig* precision_config = nullptr;
        std::optional<xla::PrimitiveType> preferred_element_type;
        op = xla::DotGeneral(lhs, rhs, dims, precision_config, preferred_element_type);
        break;
    }

    // One-argument ops:
    case AbsNode: op = xla::Abs(*inputs[0]); break;
    case NegNode: op = xla::Neg(*inputs[0]); break;
    case ExpNode: op = xla::Exp(*inputs[0]); break;
    case Expm1Node: op = xla::Expm1(*inputs[0]); break;
    case FloorNode: op = xla::Floor(*inputs[0]); break;
    case CeilNode: op = xla::Ceil(*inputs[0]); break;
    case RoundNode: op = xla::Round(*inputs[0]); break;
    case LogNode: op = xla::Log(*inputs[0]); break;
    case Log1pNode: op = xla::Log1p(*inputs[0]); break;
    case LogicalNotNode: op = xla::Not(*inputs[0]); break;
    case LogisticNode: op = xla::Logistic(*inputs[0]); break;
    case SignNode: op = xla::Sign(*inputs[0]); break;
    case ClzNode: op = xla::Clz(*inputs[0]); break;
    case CosNode: op = xla::Cos(*inputs[0]); break;
    case SinNode: op = xla::Sin(*inputs[0]); break;
    ...
    default:
        return new xla::Status(absl::StatusCode::kInvalidArgument,
            absl::StrFormat("invalid node_type=%d for ComputationAddOp", node->node_type));
    }
    if (!op.valid()) {
        ...
        return new xla::Status(absl::StatusCode::kInvalidArgument,
            absl::StrFormat("failed to convert node to XLA: node_type=%d", node->node_type));
    }
...
XlaStatus *ClientCompileComputation(Client *client, Computation *comp, int num_params, Shape **param_shapes, XlaOp *output) {
    ...
    auto status_or = client->client->Compile(*comp->xla_comp, shape_pointers, options);
    if (!status_or.ok()) {
    ...
    if (local_execs.size() > 1) {
        return new xla::Status(absl::StatusCode::kInvalidArgument,
           absl::StrFormat(
                "compilation for multiple partitions not allowed, got %d partitions, wanted only one",
...
StatusOr ClientExecuteComputation(Client *client, Computation* comp, int num_params, XlaShapedBuffer** params) {
    ...
    xla::ScopedShapedBuffer *ssb = new xla::ScopedShapedBuffer(std::move(*data_or));
...
StatusOr TransferFromServer(XlaGlobalData *gd, Client* client) {
    ...
    Literal *res = XlaLiteralToLiteral(new xla::Literal(std::move(literal_or.value())));
