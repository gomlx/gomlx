...
aotCompileCpuExecutable(std::unique_ptr<xla::HloModule> hlo_module) {
  ...
  cerr << "aotCompileCpuExecutable: Serialized to " << result.size()
       << " bytes." << endl;
  xla::cpu::XlaRuntimeCpuExecutableProto exec_proto;
  cerr << "aotCompileCpuExecutable: Parsing executable: "
       << exec_proto.ParseFromString(result) << endl;
...
aotCompileToSerialized2(stream_executor::Platform *platform,
  ...
  std::unique_ptr<xla::HloModuleGroup> hlo_module_group(
      new xla::HloModuleGroup(std::move(hlo_module)));
...
xla::StatusOr<std::string> aotCompileToSerialized3(Client *client,
  ...
  cerr << "Just before LocalClient::CompileAheadOfTime() call: " << num_params
       << " parameters." << endl;
  TF_ASSIGN_OR_RETURN(
      std::vector<std::unique_ptr<xla::AotCompilationResult>> aot_result,
      client->client->CompileAheadOfTime(*comp->xla_comp, shape_pointers,
...
StatusOr ClientAOTCompileComputation(Client *client, Computation *comp,
  ...
  if (false) {
    auto status_or =
        aotCompileToSerialized3(client, comp, num_params, param_shapes);
    if (!status_or.ok()) {
      r.status = FromStatus(status_or.status());
      return r;
    }
  ...
  if (platform_id == stream_executor::host::kHostPlatformId) {
    ...
    // auto string_or = aotCompileToSerialized2(platform,
    // std::move(hlo_module));
    if (!string_or.ok()) {
