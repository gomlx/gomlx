...
// status.h holds C/Go conversion utilities, in particular handling of xla::Status and xla::StatusOr.
...
constexpr int XlaWrapperVersion = 12;
...
// Converts std::string containing binary data to an allocated VectorData object, that
// points to a newly allocated array of bytes.
extern VectorData *str_to_bytes(const std::string &s);

// Converts a vector<string> to an allocated `VectorPointers *` that in Go can be passed to StrVectorFree.
extern VectorPointers *c_vector_str(const std::vector<std::string> &v);

// FromStatus creates a dynamically allocated status (aliased to *XlaStatus) from the given one -- contents are transferred.
