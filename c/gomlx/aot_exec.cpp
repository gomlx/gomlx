...
StatusOr NewAOTExecutable(Client *client, VectorData *serialized_aot_result) {
    ...
    string aot_result_str(static_cast<const char*>(serialized_aot_result->data), num_bytes);
    free(serialized_aot_result->data);
    free(serialized_aot_result);

//    xla::cpu::XlaRuntimeCpuExecutableProto exec_proto;
//    cerr << "NewAOTExecutable(): parsing executable -> " << exec_proto.ParseFromString(aot_result_str) << endl;
//    cerr << "\tSerialized aot result (" << num_bytes << " bytes): " << aot_result_str.size() << endl;
//
    // Create executable.
    xla::ExecutableBuildOptions executable_build_options;
    cerr << "\tClient platform: " << client->client->platform()->Name() << endl;
    auto executable_or = client->client->Load(aot_result_str, executable_build_options);
    if (!executable_or.ok()) {
...
StatusOr ExecuteAOT(Client *client, AOTExecutable *exec,
