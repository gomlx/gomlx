...
StatusOr OnDeviceBufferSubTree(OnDeviceBuffer *b, int path_length, int64_t* path) {
    ...
    if (b == nullptr) {
        ...
        xla::ScopedShapedBuffer *ssb = new xla::ScopedShapedBuffer(std::move(b->ssb_buffer->TakeSubTree(indices)));
        OnDeviceBuffer *wrapper = new OnDeviceBuffer();
...
StatusOr OnDeviceBufferToLiteral(OnDeviceBuffer *buffer, Client *client) {
    ...
    if (sb == nullptr) {
        ...
        Literal *res = XlaLiteralToLiteral(new xla::Literal(std::move(status_or.value())));
