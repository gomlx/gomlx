...
func TestSymbolicDimensionIntegration(t *testing.T) {
	...
	input := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, shapes.DynamicDim, hiddenSize).
		WithAxisName(0, "batch").WithAxisName(1, "seq")
	...
	t.Run("BinaryOps", func(t *testing.T) {
		...
		mask := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, shapes.DynamicDim, shapes.DynamicDim).
			WithAxisName(0, "batch").WithAxisName(1, "seq").WithAxisName(2, "seq")
		...
		require.Equal(t, "seq", output.AxisName(1))
	})
	...
	t.Run("Reshape", func(t *testing.T) {
		...
		output, err := ReshapeOp(input, []int{shapes.DynamicDim, shapes.DynamicDim, 8, 64})
		require.NoError(t, err)
		require.Equal(t, shapes.DynamicDim, output.Dimensions[0])
		require.Equal(t, shapes.DynamicDim, output.Dimensions[1])
	...
	t.Run("Concatenate", func(t *testing.T) {
		...
		input2 := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, shapes.DynamicDim, 256).
			WithAxisName(0, "batch").WithAxisName(1, "seq")
	...
	t.Run("Comparison", func(t *testing.T) {
		input2 := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, shapes.DynamicDim, hiddenSize).
			WithAxisName(0, "batch").WithAxisName(1, "seq")
...
func TestSymbolicDimensionEdgeCases(t *testing.T) {
	t.Run("DifferentAxisNames", func(t *testing.T) {
		// Different axis names should result in cleared axis name
		shape1 := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 10).WithAxisName(0, "batch")
		shape2 := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 10).WithAxisName(0, "seq")
		output, err := BinaryOp(OpTypeAdd, shape1, shape2)
		require.NoError(t, err)
		require.Equal(t, shapes.DynamicDim, output.Dimensions[0]) // Still dynamic
		require.Equal(t, "", output.AxisName(0))                  // Axis name cleared
	...
	t.Run("ConcatSymbolicDims", func(t *testing.T) {
		// Concatenating symbolic dimensions results in dynamic (unknown size)
		shape1 := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 10).WithAxisName(0, "batch")
		shape2 := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 10).WithAxisName(0, "batch")
...
func TestReshapeSymbolicValidation(t *testing.T) {
	t.Run("SymbolicToSymbolicWithMatchingStaticSizes", func(t *testing.T) {
		...
		input := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "batch")
		output, err := ReshapeOp(input, []int{shapes.DynamicDim, 256, 2})
		require.NoError(t, err)
		require.Equal(t, []int{shapes.DynamicDim, 256, 2}, output.Dimensions)
	})

	t.Run("SymbolicToSymbolicWithMismatchedStaticSizes", func(t *testing.T) {
		// Invalid: static portions don't match (512 != 256)
		input := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "batch")
		_, err := ReshapeOp(input, []int{shapes.DynamicDim, 256})
		require.Error(t, err)
	...
	t.Run("StaticToSymbolicDivisible", func(t *testing.T) {
		...
		output, err := ReshapeOp(input, []int{shapes.DynamicDim, 256})
		require.NoError(t, err)
		require.Equal(t, []int{shapes.DynamicDim, 256}, output.Dimensions)
	})
	...
	t.Run("SymbolicToStaticNotDivisible", func(t *testing.T) {
		...
		input := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 500).WithAxisName(0, "batch")
	...
	t.Run("FullySymbolicReshape", func(t *testing.T) {
		...
		input := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim).WithAxisName(0, "batch")
		output, err := ReshapeOp(input, []int{shapes.DynamicDim, 1})
		require.NoError(t, err)
		require.Equal(t, []int{shapes.DynamicDim, 1}, output.Dimensions)
	})
}
...
func TestGatherSymbolicDimensions(t *testing.T) {
	t.Run("SymbolicOperandDimension", func(t *testing.T) {
		...
		operand := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512, 768).WithAxisName(0, "batch")
	...
	t.Run("SymbolicStartIndicesDimension", func(t *testing.T) {
		...
		startIndices := shapes.MakeDynamic(dtypes.Int32, shapes.DynamicDim, 2).WithAxisName(0, "batch") // batch indices, 2D coordinates
	...
	t.Run("SymbolicIndexVectorDimension", func(t *testing.T) {
		...
		operand := shapes.Make(dtypes.Float32, 32, 512, 768)
		startIndices := shapes.MakeDynamic(dtypes.Int32, 10, shapes.DynamicDim).WithAxisName(1, "seq") // symbolic index vector dimension
		sliceSizes := []int{1, 256, 768}
...
func TestBroadcastSymbolicDimensions(t *testing.T) {
	t.Run("BroadcastOneToSymbolic", func(t *testing.T) {
		...
		output := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "batch")
	...
	t.Run("BroadcastSymbolicToConcrete", func(t *testing.T) {
		...
		operand := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 1).WithAxisName(0, "batch")
	...
	t.Run("BroadcastSymbolicToSymbolic", func(t *testing.T) {
		...
		operand := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "batch")
		output := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "batch")
		err := BroadcastInDimOp(operand, output, []int{0, 1})
		require.NoError(t, err)
	})

	t.Run("BroadcastDifferentAxisNames", func(t *testing.T) {
		// Different axis names - should allow (runtime will validate)
		operand := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "batch")
		output := shapes.MakeDynamic(dtypes.Float32, shapes.DynamicDim, 512).WithAxisName(0, "seq")
		err := BroadcastInDimOp(operand, output, []int{0, 1})
		require.NoError(t, err) // Different axis names assumed compatible, validated at runtime
	})
