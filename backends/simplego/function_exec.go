...
func (fe *FunctionExecutable) executeNode(backend *Backend, node *Node, execBuf *funcExecBuffers) error {
	...
	if node.IsMultiOutputs() {
		...
		// Gather captured input buffers for control flow ops (If, While, Sort)
		var capturedInputBuffers []*Buffer
		var capturedOwned []bool
		if len(node.capturedInputs) > 0 {
			capturedInputBuffers = make([]*Buffer, len(node.capturedInputs))
			capturedOwned = make([]bool, len(node.capturedInputs))
			for i, capturedInput := range node.capturedInputs {
				capturedIdx := capturedInput.idx
				capturedInputBuffers[i] = execBuf.results[capturedIdx]
				if capturedInputBuffers[i] == nil {
					return errors.Errorf("captured input %d for node %s not computed yet", i, node.opType)
				}
				// Captured inputs are never donated to the closure in this context
				// because they may be used by multiple closures or other parts of the graph.
				capturedOwned[i] = false
			}
		}

		outputBuffers, err := multiExecutor(backend, node, inputBuffers, inputsOwned, capturedInputBuffers, capturedOwned)
		if err != nil {
