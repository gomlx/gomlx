...
import ( //alt:base
	"github.com/gomlx/gomlx/pkg/core/dtypes/bfloat16" //alt:base
...
func dgCopyOutputBlockToFlat[T interface { //alt:base
	PODNumericConstraints | bfloat16.BFloat16 | float16.Float16 //alt:base
}]( //alt:base
...
func buildDotGeneralKernel[T PODNumericConstraints]( //alt:base
		...
		for range blockDim { // Loop over lhs rows:
			...
			for rhsRow := 0; rhsRow < blockDim; rhsRow += 4 { // range blockDim { // loop over rhs rows:
				...
				for ; contractingIdx < blockDim; contractingIdx++ {
					...
					rhsIdx2 := rhsIdx + 2*blockDim
					rhsIdx3 := rhsIdx + 3*blockDim
					sum0 += lhsFlat[lhsIdx] * rhsFlat[rhsIdx]  //alt:base
					sum1 += lhsFlat[lhsIdx] * rhsFlat[rhsIdx1] //alt:base
					sum2 += lhsFlat[lhsIdx] * rhsFlat[rhsIdx2] //alt:base
					sum3 += lhsFlat[lhsIdx] * rhsFlat[rhsIdx3] //alt:base
					//alt:bf16|f16 sum0 += lhsFlat[lhsIdx].Float32() * rhsFlat[rhsIdx].Float32()
					//alt:bf16|f16 sum1 += lhsFlat[lhsIdx].Float32() * rhsFlat[rhsIdx1].Float32()
					//alt:bf16|f16 sum2 += lhsFlat[lhsIdx].Float32() * rhsFlat[rhsIdx2].Float32()
					//alt:bf16|f16 sum3 += lhsFlat[lhsIdx].Float32() * rhsFlat[rhsIdx3].Float32()
					lhsIdx++
					rhsIdx++
				}
