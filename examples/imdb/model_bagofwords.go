...
func EmbedTokensGraph(ctx *model.Context, tokens *Node) (embed, mask *Node) {
	g := tokens.Graph()
	...
	maxVocab = min(maxVocab, model.GetParamOr(ctx, "imdb_max_vocab", 20_000))
	...
	tokensEmbedSize := model.GetParamOr(ctx, "imdb_token_embedding_size", 32)
	...
	wordDropoutRate := model.GetParamOr(ctx, "imdb_word_dropout_rate", 0.0)
...
func BagOfWordsModelGraph(ctx *model.Context, spec any, inputs []*Node) []*Node {
