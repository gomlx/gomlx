...
func EmbedTokensGraph(scope *model.Scope, tokens *Node) (embed, mask *Node) {
	g := tokens.Graph()
	...
	maxVocab = min(maxVocab, model.GetParamOr(scope, "imdb_max_vocab", 20_000))
	...
	tokensEmbedSize := model.GetParamOr(scope, "imdb_token_embedding_size", 32)
	embed = layers.Embedding(scope.In("tokens"), tokens, DType, maxVocab, tokensEmbedSize, false)
	embed = Where(BroadcastToShape(mask, embed.Shape()), embed, ZerosLike(embed))

	wordDropoutRate := model.GetParamOr(scope, "imdb_word_dropout_rate", 0.0)
	if wordDropoutRate > 0 {
		shape := embed.Shape().Dimensions[:len(embed.Shape().Dimensions)-1]
		dropoutMask := Ones(g, shapes.Make(DType, shape...))
		dropoutMask = layers.Dropout(scope, dropoutMask, ConstAsDType(g, DType, wordDropoutRate))
