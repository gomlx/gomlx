...
// nodeInputsCall holds the inputs used for the call to backends.Call.
...
func (ni *nodeInputsCall) String() string {
	return fmt.Sprintf("%s(f=%v, inputs=[#%s])",
		ni.Type(),
		ni.f,
...
func Call(f Function, inputs ...*Node) []*Node {
	inputNodes := []*Node{}
	inputNodes = append(inputNodes, inputs...)
	g := validateBuildingGraphFromInputs(inputNodes...)
	inputs := &nodeInputsCall{
		f:      f,
		inputs: slices.Clone(inputs),
	}
	results, err := g.currentFunc.backendFunc.Call(inputs.f, xslices.Map(inputs, func(node *Node) backends.Value { return node.outputOps[0] })...)
	if err != nil {
		panic(err)
	}
	node := &Node{
...
func (ni *nodeInputsIf) String() string {
	return fmt.Sprintf("%s(pred=[#%d], trueBranch=%v, falseBranch=%v)",
		ni.Type(),
		ni.pred.Id(),
		ni.trueBranch,
		ni.falseBranch,
...
func If(pred *Node, trueBranch Function, falseBranch Function) []*Node {
	...
	results, err := g.currentFunc.backendFunc.If(pred.outputOps[0], inputs.trueBranch, inputs.falseBranch)
	if err != nil {
		panic(err)
	}
	node := &Node{
...
func (ni *nodeInputsSort) String() string {
	return fmt.Sprintf("%s(comparator=%v, axis=%v, isStable=%v, inputs=[#%s])",
		ni.Type(),
		ni.comparator,
		ni.axis,
		ni.isStable,
...
func Sort(comparator Function, axis int, isStable bool, inputs ...*Node) []*Node {
	...
	inputNodes = append(inputNodes, inputs...)
	g := validateBuildingGraphFromInputs(inputNodes...)
	inputs := &nodeInputsSort{
		comparator: comparator,
		axis:       axis,
		isStable:   isStable,
		inputs:     slices.Clone(inputs),
	}
	results, err := g.currentFunc.backendFunc.Sort(inputs.comparator, inputs.axis, inputs.isStable, xslices.Map(inputs, func(node *Node) backends.Value { return node.outputOps[0] })...)
	if err != nil {
		panic(err)
	}
	node := &Node{
...
func (ni *nodeInputsWhile) String() string {
	return fmt.Sprintf("%s(cond=%v, body=%v, initialState=[#%s])",
		ni.Type(),
		ni.cond,
		ni.body,
		strings.Join(xslices.Map(ni.initialState, func(node *Node) string { return fmt.Sprintf("#%d", node.Id()) }), ", "),
	)
}
...
func While(cond Function, body Function, initialState ...*Node) []*Node {
	inputNodes := []*Node{}
	inputNodes = append(inputNodes, initialState...)
	g := validateBuildingGraphFromInputs(inputNodes...)
	inputs := &nodeInputsWhile{
		cond:         cond,
		body:         body,
		initialState: slices.Clone(initialState),
	}
	results, err := g.currentFunc.backendFunc.While(inputs.cond, inputs.body, xslices.Map(initialState, func(node *Node) backends.Value { return node.outputOps[0] })...)
	if err != nil {
		panic(err)
	}
	node := &Node{
