...
//	})
//	sorted := Sort(comparator, 0, false, x)
//
// Parameters:
...
func Sort(comparator *Function, axis int, isStable bool, inputs ...*Node) []*Node {
	if len(inputs) == 0 {
		exceptions.Panicf("Sort requires at least one input tensor")
	}
	if comparator == nil {
		exceptions.Panicf("Sort requires a comparator function")
	}
	if !comparator.IsClosure() {
		exceptions.Panicf("Sort comparator must be a closure (created with NewClosure)")
	}
	...
	for i, input := range inputs[1:] {
		if !input.Shape().Equal(shape) {
			exceptions.Panicf("Sort: all inputs must have the same shape, input 0 has %s, input %d has %s",
	...
	if axis < 0 || axis >= rank {
		exceptions.Panicf("Sort: axis %d out of range for rank %d", axis, rank)
	}
	...
	if err != nil {
		panic(errors.WithMessagef(err, "Sort operation failed"))
	}
...
// Example:
//
//	pred := GreaterThan(x, Const(g, float32(0)))
