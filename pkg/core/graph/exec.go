...
//
// It's a wrapper to NewExecAny, but uses generics to add a compile-time check for valid graphFn signature.
	...
	for ii := 0; ii < graphFnT.NumOut(); ii++ {
		if graphFnT.Out(ii).Kind() == reflect.Slice && graphFnT.Out(ii).Elem() == nodeType {
			if graphFnT.NumOut() != 1 {
				return errors.Errorf("[]*Node parameters are only accepted as output if they are the only output, got function type %s instead", graphFnT)
			}
...
func (e *Exec) createAndCacheGraph(argsShapes []shapes.Shape) (entry *execGraphCacheEntry) {
	...
	if e.outputAsSlice {
		...
		if !ok {
			Panicf("graphFn for %q did not return a []*Node, instead it returned %T!?",
		...
		for i, outV := range outputsV {
			...
			if !ok {
				Panicf("graphFn for %q did not return a *Node for output #%d, instead it returned %T!?",
