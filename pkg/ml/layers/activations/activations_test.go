...
func TestRelu(t *testing.T) {
	graphtest.RunTestGraphFn(t, "Relu",
		func(g *graph.Graph) (inputs, outputs []*graph.Node) {
			x := graph.Const(g, []float32{0, -1, 2, -3, 4, -5, 6})
			inputs = []*graph.Node{x}
			outputs = []*graph.Node{Relu(x)}
			return
		}, []any{
...
func TestLeakyReluWithAlpha(t *testing.T) {
	graphtest.RunTestGraphFn(t, "LeakyReluWithAlpha",
		func(g *graph.Graph) (inputs, outputs []*graph.Node) {
			x := graph.Const(g, []float32{0, -1, 2, -3, 4, -5, 6})
			inputs = []*graph.Node{x}
			outputs = []*graph.Node{LeakyReluWithAlpha(x, 0.1)}
			return
		}, []any{
...
func TestGelu(t *testing.T) {
	...
	graphtest.RunTestGraphFn(t, "GeluApproximate",
		func(g *graph.Graph) (inputs, outputs []*graph.Node) {
			x := graph.Const(g, []float32{0, -1, 2, -3, 4, -5, 6})
			inputs = []*graph.Node{x}
			outputs = []*graph.Node{GeluApproximate(x)}
			return
		}, []any{
