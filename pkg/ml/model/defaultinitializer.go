package model

import (
	"math"

	"github.com/gomlx/gomlx/pkg/core/graph"
	"github.com/gomlx/gomlx/pkg/core/shapes"
)

// VariableInitializer builds a valueNode that returns a value to initialize a variable of the given
// shape. It is defined in the Context.
type VariableInitializer = func(g *graph.Graph, shape shapes.Shape) *graph.Node

// DefaultInitializer is the default initializer for variables created with VariableWithShape.
// It is set to a "He" initializer (https://arxiv.org/pdf/1502.01852) but can be changed by the user.
//
// If you want control over the seed used for the random number generator, please use the initializer.He to create an
// initializer with a specific RNG, which can be seeded with arbitrary values..
//
// You can set a different initializer per variable with Variable.WithInitializer.
// Or you can set this global default.
var DefaultInitializer VariableInitializer = heInitializer()

// heInitializer returns the initializer that tries to preserve the variance of 1, calculated for the Relu activation functions.
//
// It creates and uses a random number generator (RNG) seeded with a system random noise.
// If you want more control (e.g.: setting the seed), please use the initializer.He to create one.
//
// It initializes biases (anything with rank <= 1) to zeros.
//
// [1] https://medium.com/@tylernisonoff/weight-initialization-for-cnns-a-deep-dive-into-he-initialization-50b03f37f53d
// [2] https://arxiv.org/pdf/1502.01852
//
// This code is copied from the initializer package, to void the dependency (loop).
func heInitializer() VariableInitializer {
	// The closure itself keeps the RNG used by the initializer.
	var stateVar *Variable
	return func(g *graph.Graph, shape shapes.Shape) *graph.Node {
		if stateVar == nil {
			stateVar = MustVariableWithValue("rng_state", graph.RngState())
		}
		if !shape.DType.IsFloat() && !shape.DType.IsComplex() {
			return graph.Zeros(g, shape)
		}
		if shape.Rank() <= 1 {
			// Zero-bias.
			return graph.Zeros(g, shape)
		}
		fanIn, _ := computeFanInFanOut(shape)
		scale := max(1.0, float64(fanIn))
		stddev := math.Sqrt(2.0 / scale)
		state := stateVar.ValueGraph(g)
		state, values := graph.RandomNormal(state, shape)
		stateVar.SetValueGraph(state)
		return graph.MulScalar(values, stddev)
	}
}

// computeFanInFanOut of a variable expected to be the parameters of
// either layers.Dense or layers.Convolution.
func computeFanInFanOut(shape shapes.Shape) (fanIn, fanOut int) {
	rank := shape.Rank()
	switch rank {
	case 0: // Scalar.
		fanIn = 1
		fanOut = fanIn
	case 1: // 1D shape, like a bias term in a dense layer.
		fanIn = 0
		fanOut = fanIn
	case 2: // 2D shape, weights of a dense layer.
		fanIn = shape.Dimensions[0]
		fanOut = shape.Dimensions[1]
	default: // Assuming convolution kernels (2D, 3D, or more):
		receptiveFieldSize := 1
		for _, dim := range shape.Dimensions[:rank-2] {
			receptiveFieldSize *= dim
		}
		fanIn = shape.Dimensions[rank-2] * receptiveFieldSize
		fanOut = shape.Dimensions[rank-1] * receptiveFieldSize
	}
	return
}
