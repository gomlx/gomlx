...
func (local *Local) DType() dtypes.DType {
...
func FlatFromRef[T shapes.Supported](ref *LocalRef) []T {
	ref.AssertValid()
	if ref.local.shape.DType != shapes.FromGoType[T]() {
		var t T
...
func (local *Local) CopyData(dst any) {
	...
	if dstT.Elem() != shapes.GoType(local.DType()) {
		panic(errors.Errorf("dst element type (%s) is incompatible with DType (%s), expected %s", dstT.Elem(), local.DType(), shapes.GoType(local.DType())))
	}
...
// type matching the tensor's DType (see shapes.GoType).
//
// See Local.LayoutStrides to calculate the offset of individual positions.
//
...
func (local *Local) FlatCopy() any {
	...
	dstV := reflect.MakeSlice(reflect.SliceOf(shapes.GoType(local.DType())), size, size)
...
func FromScalarAndDimensions[T shapes.Supported](value T, dimensions ...int) (local *Local) {
	shape := shapes.Make(shapes.FromType(reflect.TypeOf(value)), dimensions...)
	local = FromShape(shape)
	dataRef := local.AcquireData()
	defer dataRef.Release()
...
func FromFlatDataAndDimensions[T shapes.Supported](data []T, dimensions ...int) (local *Local) {
	var tmp T
	shape := shapes.Make(shapes.FromType(reflect.TypeOf(tmp)), dimensions...)
...
func FromValue[S MultiDimensionSlice](value S) *Local {
