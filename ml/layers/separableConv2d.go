...
func DepthwiseConv2D(x, kernel *graph.Node, strides []int, padding string, channelsAxisConfig images.ChannelsAxisConfig) *graph.Node {
	xShape := x.Shape()
	inputChannels := xShape.Dimensions[images.GetChannelsAxis(xShape, channelsAxisConfig)]

	// True depthwise convolution configuration
	convOpts := graph.Convolve(x, kernel).
		ChannelsAxis(channelsAxisConfig).
		StridePerDim(strides...).
		FilterGroupCount(inputChannels). // 1 filter per input channel
		PadSame()

	return convOpts.Done()
}
...
func (conv *SeparableConvBuilder) Done() *Node {
	...
	depthwiseKernelShape := shapes.Make(
		dtype,
		append(conv.kernelSize, 1, inputChannels)..., // [H,W,C,1]
	)
	...
	depthwiseOutput := DepthwiseConv2D(
		conv.x,
		depthwiseKernel,
		conv.strides,
		"SAME",
		conv.channelsAxisConfig,
	)

	// Pointwise Convolution (1x1 to combine channels)
	pointwiseKernelShape := shapes.Make(dtype, 1, 1, inputChannels, conv.filters) // [1,1,C,F]
	...
	output := graph.Convolve(depthwiseOutput, pointwiseKernel).
