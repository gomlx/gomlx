...
// ParamNormalizationType context hyperparameter defines the type of normalization to use
...
//
// Notice that this won't work for special shapes set ups.
...
var ParamNormalizationType = "normalization_type"

// NormalizeFromContext applies a normalization (or none) according to the hyper-parameter
// ParamNormalizationType configured in the context.
func NormalizeFromContext(ctx *context.Context, input *Node) *Node {
	return MaskedNormalizeFromContext(ctx, input, nil)
}

// MaskedNormalizeFromContext applies a normalization (or none) according to the hyper-parameter
// ParamNormalizationType configured in the context.
...
func MaskedNormalizeFromContext(ctx *context.Context, input, mask *Node) *Node {
	normType := context.GetParamOr(ctx, ParamNormalizationType, "layer")
	switch normType {
	case "none":
		return input
	case "layer":
	...
	default:
		Panicf("invalid normalization type %q given in context parameter %q -- valid values are 'none', 'layer' or 'batch'",
			normType, ParamNormalizationType)
	}
	return input
}
